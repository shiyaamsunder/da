# Load data
library(readxl)
diabetes <- read_excel("Documents/R/diabetes.xls")
View(diabetes)
...
model <- lm(Outcome ~ Age + BMI, data=train)
summary(model)

# Make a prediction for a new data point
new_data <- data.frame(Age = 50, BMI = 33.6)
prediction <- predict(model, newdata = new_data)
print(paste("Predicted Outcome:", prediction))

# Plot the relationship between Age and BMI and their effect on Outcome
plot(diabetes$Age, diabetes$BMI, col = ifelse(diabetes$Outcome == 1, "red", "black"), 
     xlab = "Age", ylab = "BMI")
